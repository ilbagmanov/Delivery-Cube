...
public class AdCostCallback {
    ...
    public void run(TelegramBot bot, CallbackQuery callbackQuery) {
        File file = ResourceUtils.getFile("classpath:templates/AddCost.html");
        String ans = new String(Files.readAllBytes(file.toPath()));
        accountService.updateAccountById(callbackQuery.getFrom().getId(), 3);
